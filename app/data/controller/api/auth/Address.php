...
class Address extends Auth
{
    ...
    public function set()
    {
        ...
        $province = explode('省', $data['address']);
        $city = explode('市',$province[1]);
        $area = explode('区',  $city[1]);
        $data['address'] = $area[1];
        $data['province'] = $province[0].'省';
        $data['city'] = $city[0].'市';
        $data['area'] = $area[0].'区';
    ...
    public function get()
    {
        ...
        $result = $query->order('type desc,id desc')->append(['sex_text'])->page(false, false, false, 15);
     ...
     * @return null|array
     */
    public function getCodeAddress(string $code): array
    {
          $data = $this->_vali([
            'uuid.value'   => $this->uuid,
            'code.require' => '地址编号不能为空！',
     ...
     * @return null|array
     */
    public function getAddress(string $code): array
    {
    ...
    public function getDefaultAddress()
    {
       ...
       $data = DataUserAddress::where(['uuid'=>$this->uuid,'type'=>1, 'deleted' => 0])
                                                  ->append(['sex_text'])
                                                  ->find();
