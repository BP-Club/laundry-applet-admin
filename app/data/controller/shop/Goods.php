...
class Goods extends Controller
{
    ...
    protected function _form_filter(array &$data)
    {
        ...
        if ($this->request->isGet()) {
            ...
            //处理sku，给前端显示
            $data['data_specs'] = json_decode($data['data_specs'],true);
            foreach ($data['data_specs'] as &$spec){
                foreach($spec['list'] as &$specItem){
                    $specTitle = $specItem["group"]."::".$specItem["name"];
                    $specItem['spec_code'] =  uniqid();
                    foreach ($items as &$skuItem){
                        if(strpos($skuItem['key'],$specTitle) !== false){ 
                           $specItem['price_selling'] = $skuItem["selling"] / 2;
                           if(!array_key_exists('spec_codes',$skuItem)){
                               $skuItem['spec_codes'] = [];
                           }
                           $skuItem['spec_codes'][] = $specItem['spec_code'];
                        }
                    }
                }
            }
            
            $data['data_specs'] = json_encode($data['data_specs']);
            foreach ($items as $item) data_save(ShopGoodsItem::class, [
                ...
                'spec_codes'      => json_encode($item['spec_codes']),
            ], 'goods_spec', [
                'goods_code' => $data['code'],
            ]);
        }
