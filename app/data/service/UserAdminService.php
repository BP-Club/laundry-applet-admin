...
class UserAdminService extends Service
{
    ...
    //生成小程序二维码
    public static function createQrcode($uuid,$path)
    {
        $config = [
            'appid'      => sysdata('wxapp')['appid'],
            'appsecret'  =>sysdata('wxapp')['appkey'],
            'cache_path' => app()->getRootPath() . 'runtime' . DIRECTORY_SEPARATOR . 'wechat',
        ];

        $qrcodeName = 'qrcode#'.$uuid;
        $qrcodeUrl= 'wechat/qrcode/code'.$uuid.".png";
        $data["share_uid"] = $uuid;
        $data =  base64_encode(json_encode($data,JSON_UNESCAPED_SLASHES));
        if(!Cache::store('redis')->has($qrcodeName)){
            $result = Qrcode::instance($config)->createMiniScene($data,$path, 280,true,["r" => "255", "g" => "255", "b" => "255"],false); 
            Cache::store('redis')->set($qrcodeName,$result);
        }
    }
    ...
    {
         $qrcodeName = 'qrcode#'.$uuid;
         $result = Cache::store('redis')->get($qrcodeName);
         if(empty($result)){
             return false;
         }
         return $result;
