...
abstract class PaymentService
{
    ...
    {
       // file_put_contents('pay.txt',"payok\n",FILE_APPEND);
        if(strpos($orderNo,'N') !== false){
        ...
        }else if(strpos($orderNo,'S') !== false){
            $map = ['status' => 0, 'order_no' => $orderNo];
            $order = StoreOrder::mk()->where($map)->findOrEmpty();
            if ($order->isEmpty()) return false;
            // 检查订单支付状态
            $order['status'] = 1;
            $order['pay_status'] = 1;
            ...
            $order->save();
            BrowseHistory::updateHistoryStatus($order['uuid'],0,1,$order['id'],'store_order');
        }
        return true;
    }
}