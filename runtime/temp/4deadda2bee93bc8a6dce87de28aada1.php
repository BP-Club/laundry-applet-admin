<?php /*a:1:{s:70:"/www/wwwroot/test.cloudskys.cn/ThinkAdmin/app/admin/view/api/upload.js";i:1670552399;}*/ ?>
define(['md5', 'notify'], function (SparkMD5, Notify, allowMime) {
    ...
    function UploadAdapter(elem, done) {
        return new (function (elem, done, that) {
            ...
            this.adapter = new Adapter(this.option, layui.upload.render({
                url: '<?php echo url("admin/api.upload/file"); ?>', auto: false, elem: elem, accept: 'file', multiple: this.option.mult, exts: this.option.exts.join('|'), acceptMime: this.option.mimes.join(','), choose: function (obj) {
    ...
    Adapter.prototype.done = function (ret, idx, file, done, message) {
        ...
        jQuery.post("<?php echo url('admin/api.upload/done'); ?>", {id: file.id, hash: file.xmd5});
        /*! 单个文件上传成功结果处理 */
